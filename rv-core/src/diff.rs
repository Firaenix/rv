...
fn order_entries(entries: &mut [Entry]) {
    ...
    /// — and the pair line that opens that gap (0 for the first gap, which
    ...
    /// How many lines the two files share strictly between `anchor` and
    /// `number` on the side whose one-sided entries are `dropped`: every line
    /// in between is either one of those or shared with the other side.
    fn shared_between(anchor: u32, number: u32, dropped: &[u32]) -> usize {
        ...
        let gone = dropped.partition_point(|at| *at < number)
            - dropped.partition_point(|at| *at <= anchor);
