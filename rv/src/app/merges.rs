...
//!
//! What a request carries and the thread that answers it are in [`worker`];
//! what the answer does to the reviewer's state is here.
...
impl App {
    ...
    /// rather than leaving it `Pending`: the worker holds one slot (see
    /// [`worker`]),
