...
- **Storage model** (see `docs/superpowers/specs/2026-08-17-rv-storage-model-design.md`): `comments.json` is the only file rv maintains continuously, and `REVIEW-FEEDBACK.md` is an **export** produced by `rv render`. **Saving or deleting a comment must not rewrite the markdown.** Assert on `comments.json` for persistence, and use one end-to-end `rv render` test where the exported document matters. The broader storage work — explicit reply ingest, export-staleness reporting, and retiring `snapshots/` — has its own plan; this plan only stops adding new writes to the markdown.
